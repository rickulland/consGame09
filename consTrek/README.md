...
This initial release is basically the bones of the multiplayer version with other players reduced to 'pacman' AI - some will chase, others will run away. The manual is written assuming other humans will be allowed, but your neighbors haven't reaad it.

Weird stuff first. This game is organized into rounds, with every player (real or human) being called in random order until all have played. Without knowing who's next or even who's left, the only thing to do is just play. Also, know in all turn based games, time and distance are the same thing. Warp factor 3 is both a speed and the distance covered in one turn. So shut up about the Kessel run already.

Speaking of distance, space is spacious. Looking out the window shows... nothing, usually. Instead notable surroundings are listed in grids or 'quadrants' of 8x8 sectors. The scale is distorted such that a direct route to every direction and 'depth' is a straight line. Imagine looking down upon an ancient naval chart, compass rose and ruler in hand.

Due to the extreme scaling, the apparent edge of the quadrant is not really a boundary. It's simply the point compression scaling gives up. Outside the galaxy center,  it's usually impossible to impulse to the next quadrant - keep the warp drive working. 

Let's start with the display. This is written to run on any serial text terminal without even cursor position - an Apple1 could do it. The result is a scrolling display where a 'screen' is simply the last 20 lines. Still, we'll speak of things like the main screen. You can turn on a Legend to start each turn with every player, tribe, and objects full name, or skip that and leave more of the recent action visible.

After the Legend (or not) is the primary targeting or 'Quadrant' display, using that crazy spacious space scaling idea to squash things flat. The things being squashed include big things:
...
Both groups need attention for different reasons. You can accidentally run into big things 1-5, while small things 6-7 might intentionally come after you. 
