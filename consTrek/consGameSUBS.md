...
## Commands 
       line#  #)menu ##)cmd            shipS  dist  (tmp0,tmp1)
	2000  1)Engine
		 11)warp		1	*5	1+4
		 12)impulse		2	*2	2+5
	2100  2)effector
	...
	2400  5)Debug
	  ...
	  2530   63) damage (hostile action)
          ...
          2600 7)Settings
	  2610   71) Save game
	  2620   72) Load/reload game
          2630   73) Leave game
          2640   74) Network Settings
...
3000 - AI - no plan=attack or set plan (playP)
	3010 - chase/follow
	3010 - leader also herd tribesmen
...
9002 - damage to other players dist? 
		in: glxX,glxY,pin,cmd \mod:shipS opp player
9003 - move inbounds: tmp0,1=playXY tmp2,3=oppXY tmp4=dist out: glxX,Y
		      (inc.9015,9001)
9004 - return empty testx,testy  
9005 - return damage (set rlmt=max, tmp0=ship, tmp1=start, tmp2=end)
9006 - score each player, print if cmd=23
9007 - return quadX, quadY in:testx, testy
9008 - read  resource layer - in: testx,testy out: tint(1-3)
9009 - write resource layer - in: tint(1-3) out:char (do 9000 first)
9010 - cnvt ran - in:rlmt,randl=(-1 0 +1 =seeds 1-3) out: randl=rlmt-1
9011 - print legend - 8 players, sysname
9012 - collision damage
9013 - lift player - store cargo in rint(1-3) in:testx,testy
9014 - drop player - cargo to map in:testx,y out:testx,y
