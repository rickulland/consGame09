...
A new game starts by saving a save file:-) **TerkMake.b09** creates a galaxy, inputs your name and creates 7 AI players. The resulting file can be played alone or become the seed of a multiplayer game. **TerkAdd.b09** replaces AI players with additional sites, adding one person per site and dividing the AI load between sites. A parallel program **TerkBook.b09** maintains contact info (name, hostname, IP or phone number) for each site as a separate file to share between games. 
