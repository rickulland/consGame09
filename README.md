...
**TerkGame.b09** in the program orchestrating all this, and right now it's in quite the state. What we've done is completely unbalance the game, so the AI is hyperactive and all seeing, damage is not much per attack. Doing many things is finding many bugs, so we rage quit bugsquashing and write new code pretty often. But work continues despite me. 
...
Tiny bugs, easy fix?: 
	player1 has 105% in stats 4,5,6
	some players born with max cargo, not 0
	AI doesnt impulse towards galaxtic center in survey mode
	attack damage same across row of stats
	attack damage too small

Big bugs, (redacted):
	player1 stats instantly fall to 0 at random? times (~ round 7)
